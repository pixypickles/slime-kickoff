const GAME_BUILD='v0.55-cache-fix';
'use strict';
...
let chiefModeUnlocked=false;
try{chiefModeUnlocked=localStorage.getItem('slimeKickoffChiefMode')==='1'||!!unlockState.thunder;}catch(e){chiefModeUnlocked=!!unlockState.thunder;}
...
function refreshMatchModes(){if(!ui.matchMode)return;const option=ui.matchMode.querySelector('option[value="chief"]');if(option){option.disabled=!chiefModeUnlocked;option.textContent=chiefModeUnlocked?'村長参戦モード（村長を選択）':'村長参戦モード（物語クリアで解放）';}if(ui.matchMode.value==='chief'&&!chiefModeUnlocked)ui.matchMode.value='campaign';}
...
function reset(afterGoal=false){players=[];for(let i=0;i<3;i++){const a=new Player(0,250,[225,310,395][i],i===0,i),b=new Player(1,750,[225,310,395][i],false,i);if(i===0){a.spirit=isChiefMode()?(selectedChief==='salubie'?'fire':'ice'):selectedSpirit;b.spirit=opponentElement();if(isChiefMode())a.chiefType=selectedChief;}players.push(a,b);}
...
function endGame(){running=false;ui.controls.classList.add('hidden');ui.controls.style.display='none';const win=score[0]>score[1],draw=score[0]===score[1];
 if(matchMode==='free'||matchMode==='chief'){ui.result.classList.remove('hidden');ui.result.style.display='grid';ui.resultTitle.textContent=draw?(isChiefMode()?'村長戦は引き分け！':'練習試合は引き分け！'):win?(isChiefMode()?`${selectedChief==='salubie'?'さるびえ村':'ぶりふぉ村'}村長、勝利！`:'練習試合に勝利！'):(isChiefMode()?'村長、まさかの敗北…':'練習試合に敗北…');ui.resultText.textContent=`${playerVillageName()} ${score[0]} － ${score[1]} ${enemyVillageName()}`;if(ui.nextStageBtn)ui.nextStageBtn.hidden=true;return;}
...
function startFromIntro(e){
 ...
 try{
  ...
  if(matchMode==='free'||matchMode==='chief')currentStage=clamp(Number(ui.freeOpponent&&ui.freeOpponent.value)||1,1,highestStageSeen);if(matchMode==='chief')selectedChief=ui.chiefSelect?ui.chiefSelect.value:'brifo';
...
if(ui.matchMode)ui.matchMode.addEventListener('change',()=>{matchMode=ui.matchMode.value;if(ui.freeOpponentWrap)ui.freeOpponentWrap.classList.toggle('hidden',matchMode==='campaign');if(ui.chiefSelectWrap)ui.chiefSelectWrap.classList.toggle('hidden',matchMode!=='chief');if(ui.storyDifficultyWrap)ui.storyDifficultyWrap.classList.toggle('hidden',matchMode!=='campaign');selectedChief=ui.chiefSelect?ui.chiefSelect.value:'brifo';if(ui.skillBtn)ui.skillBtn.textContent=matchMode==='chief'?(selectedChief==='salubie'?'A：拡散火球':'A：レーザー'):selectedSpirit==='plain'?'投石':selectedSpirit==='ice'?'冷気':selectedSpirit==='wind'?'竜巻':selectedSpirit==='earth'?'岩砲':selectedSpirit==='thunder'?'パチッ':'火球';if(ui.dashBtn)ui.dashBtn.textContent=matchMode==='chief'?(selectedChief==='salubie'?'熱風':'ブリザード'):'ダッシュ';});
