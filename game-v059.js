const GAME_BUILD='v0.59-chief-menu-repair';
'use strict';
...
class Player{
 ...
 kick(){
  ...
  for(const p of players){if(p===this||p.team===this.team||p.respawnT>0||p.buriedT>0||p.wallStickT>0)continue;const px=p.x-this.x,py=p.y-this.y,n=norm(px,py);if(Math.hypot(px,py)<88&&n.x*fx+n.y*fy>-.1){const easyEnemyLaunch=isEasyStory()&&this.team===0&&p.team===1?1.55:1;p.vx+=fx*power*1.28*elementPower*easyEnemyLaunch;p.vy+=fy*power*1.28*elementPower*easyEnemyLaunch;p.stun=Math.max(p.stun,sliding?.82:air?.62:.46);p.vx+=fx*(air||sliding?360:220)*easyEnemyLaunch;p.vy+=fy*(air||sliding?360:220)*easyEnemyLaunch;shake=Math.max(shake,8);burst(p.x,p.y,elementColor,12);
    ...
    if(air&&this.chiefType==='brifo'){p.frozen=Math.max(p.frozen,4);p.stun=0;p.vx*=.18;p.vy*=.18;message='空中氷撃！ 4秒凍結！';messageLife=.9;burst(p.x,p.y-p.jumpHeight(),'#bff7ff',24);continue;}
    if(air&&this.chiefType==='salubie'){p.burn=Math.max(p.burn,3.4);p.burnTick=0;p.stun=Math.max(p.stun,.45);p.vx*=.55;p.vy*=.55;message='空中炎撃！ ゴロゴロ消火中！';messageLife=1;burst(p.x,p.y-p.jumpHeight(),'#ff7a24',28);continue;}
...
function updateFireballs(dt,stopped){
 ...
 for(const f of fireballs){
  ...
  for(const target of players){
   ...
   if(Math.hypot(target.x-f.x,target.y-f.y)<target.r+f.r+7){const n=norm(f.vx,f.vy);const fp=f.chiefPower||1;target.vx+=n.x*650*fp;target.vy+=n.y*650*fp;target.burn=f.chief?3.2:1.9;target.burnTick=0;target.stun=.38;target.vx+=n.x*240*fp;target.vy+=n.y*240*fp;f.life=0;message='燃えた！ コロコロ消火中！';messageLife=1;shake=11;burst(target.x,target.y-target.jumpHeight(),'#ff7a24',28);break;}
  }
 }
...
function freezeByChief(target){
 if(target===slime){slime.frozen=Math.max(slime.frozen||0,4);slime.vx*=.08;slime.vy*=.08;slime.hop=0;burst(slime.x,slime.y,'#d9fbff',24);return;}
 target.frozen=Math.max(target.frozen,4);target.stun=0;target.vx*=.08;target.vy*=.08;burst(target.x,target.y-target.jumpHeight(),'#bff7ff',22);
}
...
function updateChiefHeatWind(dt,stopped){
 ...
 if(chiefHeatWind.hitTick<=0){
  ...
  for(const target of players){
   if(target.team===chiefHeatWind.team||target===chiefHeatWind.owner||target.respawnT>0||target.buriedT>0)continue;
   if(Math.abs(target.x-chiefHeatWind.x)<chiefHeatWind.width*.55){target.vx+=chiefHeatWind.dir*310;target.vy+=(target.y-H/2)*.18;target.burn=Math.max(target.burn,.75);target.burnTick=0;target.stun=Math.max(target.stun,.12);}
  }
...
function updateChiefIceBullets(dt,stopped){
 for(const b of chiefIceBullets){b.life-=dt;if(stopped)continue;b.x+=b.vx*dt;b.y+=b.vy*dt;b.spin+=dt*12;
  for(const target of players){if(target.team===b.team||target===b.owner||target.respawnT>0||target.buriedT>0)continue;if(Math.hypot(target.x-b.x,target.y-target.jumpHeight()-b.y)<target.r+b.r){freezeByChief(target);b.life=0;break;}}
  if(b.life>0&&Math.hypot(slime.x-b.x,slime.y-b.y)<slime.r+b.r){freezeByChief(slime);b.life=0;}
 }
